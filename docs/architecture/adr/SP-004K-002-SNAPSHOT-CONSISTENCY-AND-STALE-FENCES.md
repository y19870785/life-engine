...
K1 候选由适配器自行查询受信 Runtime，并保留原会话及查询/激活输入；快照记录独立版本与来源。组装末尾和显式 `revalidate()` 均前后核对 World/Session，中间重查 Memory、Lore、Story 和对话版本。H 将来发模型前仍须再次重验。状态：**IMPLEMENTED / PENDING_INDEPENDENT_REVIEW**；原决策不变。
