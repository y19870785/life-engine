...
**IMPLEMENTED / PENDING_INDEPENDENT_REVIEW**。J1 固定 32 本启用书、32 个 Scope 绑定、每书 10,000 条、Scope 最多扫描 10,000 条与 10,000 个触发词、16 轮、128 条输出、512 KiB 输出、16 MiB 扫描工作量等 J0 候选上限。排序集中使用轮次、常驻先于触发、优先级降序、绑定顺序、条目顺序和类型化 ID 字节。一次字面词与一个语料片段的比较按该片段 UTF-8 字节计入扫描工作量，比较前检查剩余预算；不使用耗时估算。预算耗尽返回完整条目的稳定前缀和固定诊断，不截断正文，也不跳过过长条目填入更短条目。
