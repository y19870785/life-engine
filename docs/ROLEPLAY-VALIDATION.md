...
迁移测试验证 schema 2 源库不变、副本迁移、旧备份恢复、迁移失败入口不变、缺失数据库拒绝重建。Windows 清理错误通过关闭真实 SQLite 连接修复，没有跳过业务断言。首次 GitHub Windows CI 另发现解压包测试误复制了只读 .git 对象，删除样本失败；现已排除 Git 元数据，保留删除源码后运行与记忆保留断言。
