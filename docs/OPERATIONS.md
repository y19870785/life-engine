...
v0.4 支持 schema 2 → 3：先复制数据到新代次，再事务迁移副本，最后同时切换代码和数据入口。旧代次保持原样。升级后执行 `manage.py refresh-bridges`、按实例 `connect`，并重载宿主，才能注册 `/rp`。角色卡、头像、世界书及两层记忆都在 SQLite 内随快照保存。详细命令见 [ROLEPLAY.md](ROLEPLAY.md)。
